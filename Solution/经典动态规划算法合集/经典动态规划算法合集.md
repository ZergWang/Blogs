...
# 内外层循环对排列组合的影响
先比较以下这两道题目：[LeetCode 518](https://leetcode.com/problems/coin-change-ii/) 和 [LeetCode 377](https://leetcode.com/problems/combination-sum-iv/)

这两题都是给出一个数组和一个目标值，使用数组中的元素（每个元素的数量无限）来组成目标值，问共有多少种方案。两题的区别在于前者需考虑组成目标值的元素顺序。假设数组为[1, 2, 3]，目标值为4：
