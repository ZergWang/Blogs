...
S发送packet后，如果R含ACK/NAK的回复出错了，那S无法得知其发出的packet是否存在错误。那S是重发还是不重发呢？如果重发，R可能会接受到重复的packet。

对此，可为packet加入序列号（sequence number），只要R没有正常回复ACK，S就都会重发packet。R可通过序列号找到重复的packet，直接丢弃即可。

## RDT 2.2：ACK改进
对于R来说，其回复的packet可以去掉NAK flag并加入确认号（acknowledgement number），用于对其收到的某个特定packet回复ACK。每次R收到S发送的packet后，就回复自己收到的最后一个完好的packet对应的ACK即可（此种回复称为cumulative ACK）。
...
假设将Selective Repeat的窗口大小设置为$2^{n-1}+1$（也就是3），S发送packet 0、1、2后，R顺利接收，并依次回复ACK 0、1、2，接下来可能会产生如下情况：
