...
当client发送FIN segment后，client就无法再向server发送任何数据了。首先要注意，如果client之前发送的一些segment没有收到相应的回应，在发了FIN segment之后依然有可能重传这些segment。其次要注意，client在发了FIN之后，只是不发数据了，FIN ACK这种segment还是会发的。再次，client只是关闭了发送能力，其仍可以从server处接收数据。
### 第二次挥手
