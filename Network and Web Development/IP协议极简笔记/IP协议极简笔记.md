...
Subnet Number（子网号，按本人理解也可以叫作网络号）一般为该子网下的第一个IP地址。换句话说，就是该子网下host部分全为0的那个IP地址。例如上面的114.5.14.1/23，该IP二进制为01110010.00000101.00001110.00000001，后面9位表示host，则网络部分不变，后面9位全置0就是子网号。
...
相较于IPV4，IPV6将IP地址从32位拓展到128位，且取消了报文的分片与组装，大大提高传输效率。此外，option字段被移除出header部分（在IPV6种，option内容放在拓展header中传输），让header的长度固定。
