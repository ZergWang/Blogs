...
class Solution {
public:
    stack<long long> num;  //数字栈，存储数字
    stack<char> oper; //符号栈，存储符号

    long long GetPriority(char c) {  //获取运算符的优先级
        if (c=='+'|| c=='-') 
            return 0;
        if (c=='*' || c=='/') 
            return 1;
        ...
        while (i < s.size()) {
            if (s[i] >= '0' && s[i] <= '9') {
                ...
                if (s[i] == '(') {
                    ...
                    //如果负号前面不是数字，且负号前没有运算符或者前一个运算符是左括号，说明该负号是一元运算符，应在负号前补个0
                    if (s[i] == '-' && !lastNum && (oper.empty() || oper.top() == '(')) 
                        num.push(0);
                    lastNum = 0;
                    //当前运算符的优先级小于之前运算符的优先级，就可以结算一部分之前的表达式
                    while (!oper.empty() && oper.top() != '(' && GetPriority(s[i]) <= GetPriority(oper.top()))
                        Calc();
                    oper.push(s[i]);
                }
                ++i;
