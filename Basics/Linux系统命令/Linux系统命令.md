...
```
-s表示创建为软链接（符号链接），没有-s为硬链接。不可为目录文件设置硬链接，且硬链接不能跨文件系统使用。

第一个参数为目标路径，第二个参数为该链接所处位置。

之后我们执行```cd ~/desktop```即可访问到```/mnt/c/Users/ZergWang/Desktop```。
<br/><br/>
...
终止当前前台运行的作业，但将其保留并挂起：Ctrl + z
```bash
# 查看被挂起的作业（显示结果中，每行代表描述了一个作业，行首数字为作业号）
jobs
...
buff/cache显示buffer和cache使用的内存。这两块内存起到减少磁盘读写次数，提高效率的作用。其中，buffer一般用于缓存要写入到磁盘的数据。当有多个要写入到磁盘的数据时，一般会先存储在buffer中，当buffer满了再将数据一次性写入到磁盘中，减少对磁盘的写入次数，提高效率。cache一般用于缓存读取的磁盘数据。当这些磁盘数据需要被反复访问时，就直接从cache中读取，减少对磁盘的读取次数。
