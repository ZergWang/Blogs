...
其中，total列显示系统总的内存大小。used列显示已经使用的内存。free目前空闲的内存，shared列显示被共享使用的内存，buff/cache显示buffer和cache使用的内存，available显示
...
. 脚本名
```
脚本对Shell上下文的修改在执行结束后仍会保留，例如，脚本切换了工作目录，当脚本结束后，Shell会保持在脚本切换的工作目录中。sh命令由于在新fork的子进程中执行脚本，因此脚本的操作不影响当前的Shell。
