...
- **前台/后台进程**：前台进程运行于当前与用户交互的终端上，占用终端，且输出的信息都会显示在屏幕上。如果终端异常关闭，进程也会被终止。后台进程不占用终端，在系统中默默运行，不受到当前终端的影响，进程输出的消息也不会显示到屏幕上。
...
单个管道其实可以实现半双工通信，只要多个进程不同时使用管道的读端或写端，则不会发生冲突。
### 匿名管道
