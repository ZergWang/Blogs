...
## 概念解析
临界资源：一次只能被一个进程访问的共享资源被称为临界资源。

临界区（critical section）：访问临界资源的程序被称为临界区。上述对共享变量加1的程序就属于临界区。

竞争：多线程并发执行时，临界区可同时被多个线程执行，例如上述线程A、B的运行方式，这会导致执行结果的不确定性。

互斥：多线程并发执行时，同一时间只能有一个线程执行临界区程序。如果上述线程A、B以互斥方式运行，就能得到正确的结果。

同步：若多个进程/线程在并发执行时相互配合，并一些特殊节点上会相互等待互通消息，而不是各自为战只顾自己的运行，这种称为进程/线程的同步。（与之相对的概念称为“异步”。在异步执行下，进程/线程只按照自己的顺序逐个执行命令，不会阻塞等待其他进程/线程的结果）。
...
当线程以读的形式访问资源时，先检查资源上是否存在锁。如果无锁或只有读锁，则可以访问，并对资源加上自己的读锁，否则阻塞，等待锁释放。读锁允许与其他读锁共存。也就是说，资源允许同一时间有多个线程读取资源。

当线程以写的形式访问资源（也就是更改资源）时，同样要检查锁。只有无锁的情况下才可访问资源，并对资源加上写锁。写锁不能与任何锁共存。多个线程应以互斥的形式更改资源。
...
#### 死锁
##### 死锁的概念与条件
进程A和B都要申请资源1和2，假设进程A先占有了资源1，进程B占有了资源2，此时A需要等待B释放资源2，B需要等待A释放资源1，两个进程互相无限等待下去，就陷入死锁状态。
...
##### 避免死锁
因此，以上四个条件只要令任意一个条件不成立，就能避免死锁。较为简单的方法是通过有序申请资源的方法来破坏循环等待条件。例如，假设进程A和B都要申请资源1和2，则两进程必须按 1 -> 2的顺序申请这两个资源，从而避免进程A占有了资源1，进程B占有资源2陷入死锁的情况。
...
### 信号量
在进程通信部分已经描述了信号量的机制，相比于锁，信号量机制更为复杂，功能也更为完善，可以实现互斥、同步等多种关系。
#### 使用信号量实现互斥
对于共享资源，将其对应的信号量初始化为1即可。进程访问共享资源前执行P操作，访问结束后执行V操作即可。
...
#### 生产者-消费者模型
生产者进程用于生产数据，并将数据放入缓冲区中。消费者进程用于使用数据，每次从缓冲区中拿取数据。对此，需要实现以下三种互斥同步关系：
...
- 读优先：线程A事先读取了资源，然后线程B需要更改资源。由于读锁的存在，线程B阻塞，等待A释放读锁。此时允许新的线程来读取资源。这样一来，线程B需要一直阻塞，直到某个时刻没有读锁时才能访问。读优先的实现方式可能会导致写线程的饥饿。
- 写优先：在上述场景下，当写线程阻塞，等待读锁释放时，不允许新的线程来读取资源。这同样可能导致读线程的饥饿。
- 公平读写：不管是读线程还是写线程。
<br/><br/>
