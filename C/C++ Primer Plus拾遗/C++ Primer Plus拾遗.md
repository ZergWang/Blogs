...
实际上，编译器根据依次按照下面几种规则去进行类型转换，如果转换后匹配到了唯一的函数，则执行函数，否则继续按下一条规则继续类型转换：

1. 精确匹配：不转换，或者只作微不足道的转换，如数组名到指针、函数名到函数指针、T到const T；
2. 整型提升：bool、char、short会统一提升到int、float会提升到double；
3. 准转换匹配：如int和double的相互转换。
...
另外，根据上述类型转换的第一条规则，对于const类型的形参，有const以及无const的实参都能匹配。但是反过来，const类型的实参不会匹配非const类型的形参。
