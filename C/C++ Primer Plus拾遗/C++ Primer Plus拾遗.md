...
```
#### 函数重载
C++支持函数重载，允许定义名字相同的函数，只要其参数表不同即可。
...
```
#### 应用
一般会将引用变量作为函数参数或返回值使用，可起到简化程序的作用：
```cpp
void Swap(int &a, int &b) {
    //相较于指针，引用变量在声明时必须初始化，因此其必不为空，此处无需判空
    int temp = a;
    a = b;
    b = temp;
}
...
- 若实参为普通变量，引用变量在函数中不会另外创建一个副本。若传入的实参为字面量或表达式（例如a+3），或者类型与形参不符的变量，则会生成一个值与实参相同的临时变量，再让形参引用该临时变量。由于形参的引用对象并非实参，这可能导致潜在的错误。
