<font color='ff0000'>**本博文整理了C++ Primer Plus前九章中的部分知识点，一般为不常用的小技巧或基础概念性的内容。**</font>
<br/>
...
# 函数
## 内联函数
内联函数的作用和宏类似，在调用内联处插入函数的代码段，以此实现更高的执行效率。
...
一般高效但低频调用的函数会被改造为内联函数。实际上，和C语言中的register类似，即使用户将函数定义为内联函数，若函数代码过长，或者函数中存在递归，编译器也不会将其作为内联函数编译。
...
```
## 函数重载
C++支持函数重载，允许定义名字相同的函数，只要其参数表不同即可。（返回值类型相不相同无所谓）
...
一个指针类型的常量，称为指针常量。该指针无法被修改（永远指向初始化指定的变量），但其指向的内容可以随意修改。
```cpp
const int * p1;        //常量指针
int const * p2;        //同p1
...
## 引用变量
### 性质
在使用上，引用变量可视为变量的“别名”，对两者取值、取地址均一致。
...
//等价于int * const p = &a;
```
因此，和指针常量一样，引用变量需要在声明时初始化。

和指针常量不同的是，在使用引用变量时，“\*”运算符会被隐式调用。若显式使用“\*”则会报错：
```cpp
b = 20;
//等价于 *p = 20，若语句为*b = 20则报错
```
### 应用
一般会将引用变量作为函数参数或返回值使用，可起到简化程序的作用，主要有以下三点好处：
- 简化参数：指针降维。
- 无需判空：相较于指针，引用变量在声明时必须初始化，因此其必不为空，此处无需判空。
- 节省空间：若实参为普通变量，引用变量在函数中不会另外创建一个副本。
```cpp
...
当然，在使用时需要注意：
- 在函数中对引用变量的更改会影响到实参（若不希望实参被影响，可对引用变量加入const关键字加以限制）
- 实参为普通变量时引用变量在函数中不会另外创建副本。若传入的实参为字面量或表达式（例如a+3），或者类型与形参不符的变量，则会生成一个值与实参相同的临时变量，再让形参引用该临时变量。由于形参的引用对象并非实参，这可能导致潜在的错误。
