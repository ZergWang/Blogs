<font color='ff0000'>**本博文整理了C++ Primer Plus前八章中的部分知识点，一般为不常用的小技巧或基础概念性的内容。**</font>
<br/>
...
默认情况下函数也具有外部链接性，跨文件调用函数时也可通过“extern”关键字指定。若要函数仅在其文件中生效（内部链接性），和静态变量一样，加入“static”关键字即可。
<br/><br/>



# const
const关键字一般用于定义一个普通常量，使其值在运行过程中不被改变。此外，const关键字有static的作用，令常量具有内部链接性。
#### 常量指针与指针常量

指向常量的指针，称为常量指针。指针本身可以被修改，但不可通过该指针改变其指向的内容。

一个指针类型的常量，称为指针常量。该指针无法被修改（用于都是其初始化时的值），但其指向的内容可以随意修改。
```cpp
const int * p1;        //常量指针
int const * p2;        //同p1
