...
this指针不是类的成员。this指针在执行成员函数时被创建，成员函数结束时被销毁，因此对象无需存储this指针。
...
# 类的大小
## 普通类的大小
类的大小等于其所有非静态成员的大小之和。类的对象基本也等于其所有非静态成员的大小之和（包括继承自基类的成员，这些成员也存储在派生类对象的地址处），如果有虚函数，对象的大小还要算上虚表指针。


## 空类的大小
空类的大小恒为1，即使实例出对象，对象的大小也为1。（为了区分每个对象，每个对象都有自己独有的内存地址，因此对象的大小最小为1）。
```cpp
class A {};
class B: public A {};
int main() {
    A a;      
    cout << sizeof(A) << sizeof(a);// 输出11
    B b;
    cout << sizeof(B) << sizeof(b);// 输出11
}
```
类B继承了一个空类，但编译器通过优化，令类B的大小不受空的基类影响。
<br/><br/>
