...
## 友元关系
友元关系可以让一个类的私有以及保护成员被另一个不相关的类或普通函数访问。在C++中友元关系通过友元类、友元函数以及友元成员函数实现。

友元关系是单向一对一的。也就是说，类A声明了B（B可以是一个类，也可以是一个函数）是A的友元，仅仅代表B可以访问A的所有成员，但反过来不行。另外，A的任何基类和派生类也不继承这个友元关系，B无法访问A的基类或派生类的成员。B的基类或派生类同样也不能访问A的成员。

### 友元类
将类B声明为类A的友元类，则在类B的成员函数中可以直接访问类A的所有成员。
...
### 友元函数
一个类可以将一个普通函数声明为该类的友元函数。友元函数并不是该类的成员函数，因此友元函数不受访问控制符的限制。例如下面的例子，即使声明在private下，该函数也可以在类外正常调用。
```cpp
class A {
private:
    int value = 10;
...
### 友元成员函数
友元类可以让一个类完全访问另一个类的所有成员，这可能造成风险。对此，可以只让类的某个成员函数能完全访问另一个类的所有成员，也就是声明一个友元成员函数：
```cpp
class A;    //注意友元声明与友元定义之间的互相依赖
...
class A {
    int value;
    friend void B::show(A &a);   //将B的成员函数show声明为A的友元成员函数
};

void B::show(A &a) {     //只有在定义类A后才能定义该函数，毕竟，它被设为友元是为了访问类A的成员
    cout << a.value << endl;
}
```
B::show()是B的成员函数，即使被A声明为它的友元成员函数，也不会成为A的成员函数，A中对友元成员函数的访问修饰符是无效的。

使用友元成员函数时需要注意多个类的声明和定义顺序。上述代码给出的顺序是正确的（可正常执行不报错），应首先声明A，其次声明并定义B，再次定义A，最后给出友元成员函数的定义。
...
纯虚函数的声明：虚函数的声明后加上“=0”。声明了纯虚函数，或者继承了基类的纯虚函数但并没有重写的类都是抽象类，抽象类不能实例化对象。

因此，如果要某个类B强行实现某个接口，可以在抽象类A中将这个接口定义为纯虚函数。这样一来，让B作为A的派生类，B就必须要重写接口，否则B也成为抽象类，就不能实例化对象。
```cpp
class A {
public:
    virtual void func() = 0; 
...
抽象类虽然无法实例化出对象，但也有自己的构造函数。抽象类作为基类时，派生类实例化也会正常执行抽象类的构造函数。
...
编译时就能令编译器确定要执行的函数版本，称为静态绑定；代码运行时才能确定要执行的函数版本，称为动态绑定。C++允许基类指针指向派生类对象；基类引用也可直接引用派生类对象。反过来则不行。如果通过基类指针或引用调用虚函数，则会进行动态绑定。此时，编译器会认为基类指针指向的是一个基类对象，但由于调用的是虚函数，编译器会在指向的地址首位找到虚表指针，并查找虚表，然后执行对应的函数版本。但由于实际上指针指向的是一个派生类对象，查找的其实是派生类的虚表，最终执行的虚函数也是派生类的版本。

但如果通过基类指针调用的虚函数，在基类中并没有声明为虚函数，则无法动态绑定。基类中名为p的函数不是虚函数，编译器就不会访问对象地址的首位的虚表指针，因此采用静态绑定。
```cpp
class A {
public:
    void p() { cout << 'a'; }
...
int main() {
    A* p = new B;
    p->p(); //静态绑定，执行基类的成员函数，输出a
}
```
对于派生类对象，可以通过类作用域运算符来指定调用本身或者某个基类版本的函数（反过来不行，即使因为在找不到），此时采用静态绑定。
```cpp
//假设B继承自A，它们都有同名虚函数p
A* pa = new B;
pa->B::p(); //非法，编译器认为当前指向的对象类型为A，A类以及A的基类中找不到类B
A a;
B b;
a.B::p();  //非法，理由同上
b.A::p();  //合法
...
**综上所述，只有通过基类指针/引用调用一个在基类中声明为virtual的函数时，才会使用动态绑定机制。通过对象调用虚函数或非虚函数，或者通过类作用域符指定要调用的函数版本，都会使用静态绑定机制。对于非面向对象的函数多态，如函数重载、函数模板等，也是使用静态绑定机制。**
<br/>

## 成员与非虚函数的寻找机制

非虚函数的调用不涉及虚表机制。当调用某个成员或非虚成员函数时，编译器会根据成员名/函数名在当前类中递归查找（如果通过基类指针调用，就从基类中开始查找）。首先编译器会在当前类的声明中查找是否有同名的，如果找不到，则从当前类的基类开始，向上递归查找，直到找到同名成员或函数。

当找到同名函数后，编译器停止递归，检查参数表等是否匹配，如果不匹配则直接报错，不会继续递归。同理，当找到同名成员后，编译器也停止递归。如果找到的成员的类型与实际要求并不匹配，也直接报错。
```cpp
class A {public: int t = 2;};

class B: public A {public: string t = "aaa";};
...
```
如上面的代码所示，从类B开始递归查找名字为“t”的成员。在类B中找到了则直接停止递归。但由于成员类型在使用上不符，程序报错。
## 构造函数的多态
