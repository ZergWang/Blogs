...
**综上所述，只有通过基类指针/引用调用虚函数时，才会使用动态绑定机制。通过对象调用虚函数或非虚函数，或者通过类作用域符指定要调用的函数版本，都会使用静态绑定机制。对于非面向对象的函数多态，如函数重载、函数模板等，也是使用静态绑定机制。**
<br/>

## 成员与非虚函数的寻找机制

非虚函数的调用不涉及虚表机制。当对象调用某个成员或非虚成员函数时，编译器会根据成员名/函数名开始递归查找。首先编译器会从该对象的类声明中查找是否有同名的，如果找不到，则从其基类的声明开始，向上递归查找，直到找到同名成员或函数。

当找到同名函数后，编译器停止递归，检查参数表等是否匹配，如果不匹配则直接报错，不会继续向上递归寻找。同理，当找到同名成员后，编译器也停止递归。如果找到的成员的类型与实际要求并不匹配，也直接报错。
```cpp
