...
1. 对于vector容器，如果新增元素导致容器扩容（即容器的capacity变更），则所有元素的迭代器都会失效。在不触发扩容的前提下，在容器内部增删元素后，容器会移动被增删元素后的所有元素，这些元素的迭代器会失效。
2. deque在不同平台有不同的实现。对于其中一种实现，在首尾增删元素仅导致首尾元素的迭代器失效。如果在内部增删元素，如果增删的元素离头部近，容器会移动被删元素之前的元素，这会导致被删元素之前元素的迭代器失效。如果增删的元素离尾部近，同理会导致被删元素之后元素的迭代器失效。
