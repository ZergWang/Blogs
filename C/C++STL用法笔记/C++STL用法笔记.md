...
## 迭代器
迭代器可以视为一种访问容器中元素的指针。不同的容器有其对应类型的迭代器。如果要在代码中使用迭代器相关内容，需调用```<iterator>```库。
### 迭代器分类
一般容器中常见以下几对迭代器：
```cpp
begin, end 
...
在使用上，迭代器和指针非常类似。可对迭代器使用解引用符号“*”来访问元素本身，也可使用“->”访问元素的指定成员。迭代器也可使用用自增自减符号（++或--）来移动迭代器，从而遍历容器中的元素。例如下面的代码可用于遍历unordered_map，其中变量i即为unordered_map类型的迭代器。
```cpp
...
迭代器之间可使用“==”或“!=”来判断是否相等，但不可使用大于小于号比较大小。另外，对迭代器加减一个整数k会和移动指针一样，得到向前或向后的第k个元素的迭代器。迭代器之间可以相减，从而计算出迭代器对应的两元素之间的距离。STL库中有distance函数可以专门用来计算两个迭代器之间的距离：
```cpp
distance(iter x, iter y)
//返回值为迭代器y与迭代器x的距离

a.size() == distance(a.begin(), a.end())
//对于任意类型的容器a，该返回值等于容器a的size
```
