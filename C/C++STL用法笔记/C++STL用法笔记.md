...
1. 对于vector容器，push_back会导致end迭代器失效。如果本次push_back导致容器扩容（即容器的capacity变更），则所有元素的迭代器都会失效。在容器内部增删元素后，容器会移动被删元素后的所有元素，因此被删元素后的所有元素迭代器也会失效。
2. 对于deque容器，在首尾增删元素仅导致首尾元素的迭代器失效。如果在内部增删元素，如果增删的元素离头部近，容器会移动被删元素之前的元素，这会导致被删元素之前元素的迭代器失效。如果增删的元素离尾部近，同理会导致被删元素之后元素的迭代器失效。
...
max_load_factor() 
//返回当前容器的负载因子，当容器桶中平均元素数量超过该值会扩充
max_load_factor(k) 
//将当前容器的负载因子更改为k
rehash(k)
