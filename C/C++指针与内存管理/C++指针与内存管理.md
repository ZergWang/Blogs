# C/C++数据存储区域
C/C++程序运行时的各种数据会存储在不同的内存区域中，其中代码段和数据段的内存分配在程序编译时完成，堆栈的内存分配在程序运行时完成。

## 代码段
实际上被称为.text段。该段中存储编译成机器码的代码，各种函数（比如main等）都被存储在这一区域中。类的静态与非静态成员函数也会被存储在该区域中。

代码段的内存分配完成后就处于只读状态，不允许对其内部数据作修改。

## 数据段
一般又能进一步分为全局/静态区和常量区，其中全局/静态区存储全局变量、static关键字的变量，常量区存储字符串常量、虚函数表等。

## 堆（Heap）
存储使用malloc或new手动分配内存的变量。
## 栈（Stack）
存储函数中定义的临时变量，这些变量的内存空间由编译器分配和销毁，无需程序员额外管理。

注意：const关键字不改变变量存储的区域。也就是说，一个含const关键字的变量和其无const版本的变量，存储的位置是一样的。
