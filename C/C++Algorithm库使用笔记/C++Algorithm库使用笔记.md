...
# 去除数组中的重复元素
unique函数可用于去除数组中相邻的重复元素，因此使用unique前需要先对数组排序。
```cpp
unique(first, last)
//first，last为迭代器或指针，对容器[first, last)范围内，去除相邻的重复元素
//unique不改变数组长度，所有去重后的元素会存储在数组的前部，函数返回值为最后一个唯一元素再后一个元素的指针或迭代器。

//举个例子，假设对普通数组int a[]去重：
int m = unique(a, a+n) - a; //函数结果减去数组首元素地址，可得到去重后的实际长度

//对于vector<int>，有：
int m = unique(a.begin(), a.end()) - a.begin()
//函数结果减去数组首元素的迭代器，可得到去重后的实际长度
```
